...
exports.createClub = async (req, res) => {
    const{club_name, description, category, contact_email, contact_phone, club_sns} = req.body;
    // 필수 필드가 모두 있는지 확인
    if (!club_name || ! contact_phone|| !description || !contact_email || !category ||!club_sns) {
    ...
    try{
        const club = await Club.createClub({club_name, description, category, contact_email, contact_phone, club_sns});
...
exports.updateClubById = async (req, res) => {
    ...
    try {
        const club = await Club.updateClubById(club_id, { club_name, description, category, contact_email, contact_phone, club_sns });
...
//클럽 내용 수정
exports.upadateApproveMember = async (req, res) => {
    const { club_id, user_id } = req.params;ㅉ
    try {
        const club = await Club.upadateApproveMember(club_id, user_id);
        res.status(200).json(club); // 클럽 데이터 반환
